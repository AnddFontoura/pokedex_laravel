...
class getPokemonInformationCommand extends Command
{
    ...
    public function handle()
    {
        ...
        for ($i = 1; $i <= $this->valorPreDefinido; $i++) {
            ...
            $urlType1 = $pokemonData->types[0]->type->url;
            $explodeUrl = explode('/', $urlType1);
            $typeId1 = $explodeUrl[sizeof($explodeUrl) - 2];

            $urlType2 = $pokemonData->types[1]->type->url ?? null;
            if ($urlType2) {   
                $explodeUrl = explode('/', $urlType2);
                $typeId2 = $explodeUrl[sizeof($explodeUrl) - 2];
            }

            $pokemonImage = $pokemonData->sprites->versions->{'generation-vii'}->{'ultra-sun-ultra-moon'}->front_default ?? null;
